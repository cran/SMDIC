...
maf2matrix <- function(maffile,percent=0.01,nonsynonymous=TRUE) {
  maf<-read.delim(maffile,header = TRUE,comment.char = '#',stringsAsFactors = FALSE)
  mafneed<-maf[,c("Hugo_Symbol","Tumor_Sample_Barcode","Variant_Classification")]
  if (nonsynonymous==TRUE) {
    mafmissense<-mafneed[which(mafneed$Variant_Classification=="Missense_Mutation"|mafneed$Variant_Classification=="Frame_Shift_Del"|mafneed$Variant_Classification=="Frame_Shift_Ins"|mafneed$Variant_Classification=="In_Frame_Del"|mafneed$Variant_Classification=="Nonsense_Mutation"|mafneed$Variant_Classification=="In_Frame_Ins"|mafneed$Variant_Classification=="Splice_Site"|mafneed$Variant_Classification=="Nonstop_Mutation"|mafneed$Variant_Classification=="Translation_Start_Site"),]
  ...
  mafmatrix<-mafselectmatrix[rowSums(mafselectmatrix)>=cutmutnum,]
  return(mafmatrix)
}
